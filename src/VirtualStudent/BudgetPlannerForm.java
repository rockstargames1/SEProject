    ...
    private void updateBudget() {
        try {
            ...
            if(l >= 7)
            {
                ...
                sql = "update updatetime set updttime=" +y;
                System.out.println(sql);
                db.stmt.executeUpdate(sql);
                
                //update the budget
                
                sql = "select amount from budget where use='acc_bal' or use='weeklybudget'";
                System.out.println(sql);
                ResultSet rs1 = db.stmt.executeQuery(sql);        
                rs1.next();
                int bal = rs1.getInt(1);
                rs1.next();
                int bdgt = rs1.getInt(1);
                System.out.println(bal+":"+bdgt);
                if(bal >= bdgt)
                {
                    //Update Balance
                    
                    sql = "update budget set amount=amount-"+bdgt+" where use='acc_bal'";
                    System.out.println(sql);
                    db.stmt.executeUpdate(sql);
                    
                    //Update remaining amount
                    
                    sql = "update budget set amount=amount+"+bdgt+" where use='rem_amt'";
                    System.out.println(sql);
                    db.stmt.executeUpdate(sql);
                }
                else
                {
                    ...
                    JOptionPane.showMessageDialog(rootPane, "Not enough balance in your account");
